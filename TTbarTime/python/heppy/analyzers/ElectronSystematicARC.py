...
class ElectronSystARC(Analyzer):

    def __init__(self, cfg_ana, cfg_comp, looperName):
        super(ElectronSystARC, self).__init__(cfg_ana, cfg_comp, looperName)
    ...
    def process(self, event):
        ...
        for elec in electrons:
            if(elec.pt()>10 and elec.pt()<500 and elec.superCluster().eta() >= -2.5 and elec.superCluster().eta() <= 2.5):
                syst_id_weight *= self.mc_syst_id_hist.GetBinError(eta_binned(elec.superCluster().eta()), pt_binned(elec.pt()))
                if(elec.pt()>20):
                    syst_reco_weight *= self.mc_syst_reco_hist.GetBinError(eta_binned_reco(elec.superCluster().eta()), pt_binned_reco(elec.pt()))
