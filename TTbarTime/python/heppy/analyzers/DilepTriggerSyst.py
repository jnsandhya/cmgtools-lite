    ...
    def process(self, event):
        ...
        for dilep in dilepton:
            if(dilep.pt_lead() <= 200 and dilep.pt_sublead() <= 200): 
                sf_ee_trig_weight *= self.mc_sf_ee_trig_hist.GetBinError(self.mc_sf_ee_trig_hist.FindBin(dilep.pt_lead(), dilep.pt_sublead()))
                sf_em_trig_weight *= self.mc_sf_em_trig_hist.GetBinError(self.mc_sf_em_trig_hist.FindBin(dilep.pt_lead(), dilep.pt_sublead()))
                sf_mm_trig_weight *= self.mc_sf_mm_trig_hist.GetBinError(self.mc_sf_mm_trig_hist.FindBin(dilep.pt_lead(), dilep.pt_sublead()))
